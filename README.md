...
### _Instance Selection_ ### 
  ...
  * G 타입 (Shared Memory)
    ...
    * OS 스케줄링에 따라 지연 시간이 튀는 Jitter가 발생할 수 있음 (NVLink 지원되지 않음)
  ...
  * P 타입 (GPUDirect RDMA)
    ...
    * NVIDIA Aerial SDK가 요구하는 1ms 미만의 엄격한 TTI(Transmission Time Interval)를 안정적으로 맞추려면 이 하드웨어적 직접 경로가 필수적.
